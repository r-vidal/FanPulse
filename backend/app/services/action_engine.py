...
class ActionEngine:
    ...
    async def generate_actions_for_artist(
        ...
        # Get historical data
        history = self._get_history(artist_id, days=30)
        recent_history = self._get_history(artist_id, days=7)
        ...
        # Apply rules
        actions.extend(self._rule_no_data(artist, history, user_id))
        actions.extend(self._rule_insufficient_data(artist, history, user_id))
        actions.extend(self._rule_declining_momentum(artist, momentum, user_id))
        actions.extend(self._rule_fire_momentum(artist, momentum, user_id))
        actions.extend(self._rule_stale_snapshot(artist, recent_history, user_id))
        actions.extend(self._rule_high_popularity(artist, current_stats, user_id))
